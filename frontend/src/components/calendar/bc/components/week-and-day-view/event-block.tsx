...
const calendarWeekEventCardVariants = cva(
  "flex select-none flex-col gap-0.5 truncate whitespace-nowrap rounded-md border px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring",
  {
    ...
    defaultVariants: { color: "blue-dot" },
  }
)
...
export function EventBlock({ event, className }: IProps) {
  const { badgeVariant, onEventClick, hour12 } = useCalendar()
  ...
  const classes = cn(calendarWeekEventCardVariants({ color, className }), durationInMinutes < 35 && "py-0 justify-center")
    ...
    <DraggableEvent event={event}>
      ...
      >
        ...
        {durationInMinutes > 25 && (
          <p>
            {format(start, hour12 ? "h:mm a" : "HH:mm")} - {format(end, hour12 ? "h:mm a" : "HH:mm")}
          </p>
        )}
      </div>
    </DraggableEvent>
