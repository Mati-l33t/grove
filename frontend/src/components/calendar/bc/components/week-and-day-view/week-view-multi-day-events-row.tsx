...
export function WeekViewMultiDayEventsRow({ selectedDate, multiDayEvents }: IProps) {
  ...
  const weekStartOpts = { weekStartsOn: firstDayOfWeek as 0 | 1 }
  const weekStart = startOfWeek(selectedDate, weekStartOpts)
  const weekEnd = endOfWeek(selectedDate, weekStartOpts)
  ...
  const processedEvents = useMemo(() => {
    return multiDayEvents
      .map(event => {
        ...
        return { ...event, adjustedStart, adjustedEnd, startIndex, endIndex }
      })
      .sort((a, b) => {
        ...
        {weekDays.map((day, dayIndex) => (
          <div key={day.toISOString()} className="flex h-full flex-col gap-1 py-1">
            {eventRows.map((row, rowIndex) => {
              ...
              if (!event) return <div key={`${rowIndex}-${dayIndex}`} className="h-6.5" />
