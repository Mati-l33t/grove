import {
  ...
  isSameWeek, isSameDay, isSameMonth, startOfWeek, startOfMonth,
  endOfMonth, endOfWeek, format, parseISO, differenceInMinutes,
  eachDayOfInterval, startOfDay, differenceInDays, endOfYear,
...
export function hexToEventColor(hex: string): TEventColor {
  return HEX_TO_COLOR[hex.toLowerCase()] ?? "blue"
}
...
export function getEventsCount(events: IEvent[], date: Date, view: TCalendarView): number {
  const compareFns = {
    ...
    year: isSameYear,
    day: isSameDay,
    week: (a: Date, b: Date) => isSameWeek(a, b),
    month: isSameMonth,
  }
...
export function getCalendarCells(selectedDate: Date, weekStartsOn: 0 | 1 = 0): ICalendarCell[] {
  ...
  const getFirstDayOfMonth = (year: number, month: number) => {
    ...
    return weekStartsOn === 1 ? (raw === 0 ? 6 : raw - 1) : raw
  }
